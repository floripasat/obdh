...
uint8_t imu_setup(void) {
    debug_print_event_from_module(DEBUG_INFO, IMU_MODULE_NAME, "Initializing...");
    debug_new_line();
    ...
    if (who_am_i == IMU_WHO_AM_I_VALUE) {                   // if the read value is the expected
        ...
        debug_print_event_from_module(DEBUG_INFO, IMU_MODULE_NAME, "Status is OK!");
        debug_new_line();
    }
    else {
        status = IMU_NOT_WORKING;                           // IMU is not OK
        debug_print_event_from_module(DEBUG_ERROR, IMU_MODULE_NAME, "Status is NOT OK!");
        debug_new_line();
    }
...
uint8_t imu_read(uint8_t *p_imu_data, uint8_t imu_select)
{
    ...
    if (i2c_send(IMU_BASE_ADDRESS, MPU9150_ACCEL_XOUT_H, NO_STOP) == I2C_FAIL) {
        ...
        debug_print_event_from_module(DEBUG_ERROR, IMU_MODULE_NAME, "Error writting to the device!");
        debug_new_line();
    }
    ...
    if (i2c_receive_burst(IMU_BASE_ADDRESS, p_imu_data, 14, START_STOP) == I2C_FAIL) {
        imu_status = IMU_NOT_WORKING;
        debug_print_event_from_module(DEBUG_ERROR, IMU_MODULE_NAME, "Error reading the device!");
        debug_new_line();
    }
